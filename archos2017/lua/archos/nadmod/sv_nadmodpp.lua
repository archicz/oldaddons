...
if not NADMOD.Props then
	...
	for k,v in pairs({toggle=false,use=false,adminall=true,autocdp=0,autocdpadmins=false}) do
...
function NADMOD.CanTool(ply, tr, mode)
	...
	if !NADMOD.PlayerCanTouch(ply, ent) then
		if not ((NADMOD.Props[ent:EntIndex()] or {}).Name == "W" and (mode == "wire_debugger" or mode == "wire_adv" or mode == "faceposer")) then 
			return false
		end
