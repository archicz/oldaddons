...
ARCHOS.LevelingSystem.Levels[10] = 10500
...
if SERVER then
	...
	function ARCHOS.LevelingSystem.CheckPlayer(ply)
		local query = ARCHOS.db:query("SELECT * FROM levelingsystem WHERE steamid = '" .. ply:SteamID64() .. "';")
	...
	hook.Add("PlayerInitialSpawn","ARCHOS_LevelingSystem_Spawn",function(ply)
		ARCHOS.LevelingSystem.CheckPlayer(ply)
	end)
	
	ARCHOS.Hook.Add("Initialize","LevelingSystem_CheckForTable",function()
		ARCHOS.LevelingSystem.CheckForTable()
	end)
	
	concommand.Add("archos_levelingsystem_repair",function(ply,cmd,args)
		if IsValid(ply) then return end
		
		ARCHOS.LevelingSystem.CheckForTable()
		for k,v in pairs(player.GetAll()) do
			ARCHOS.LevelingSystem.CheckPlayer(v)
		end
	end)
end
